...
sub check_RNAplex {
    ...
    (open(RD, "$FindBin::RealBin/RNAplex --version 2>&1 |")) || return 0;
    my $one = <RD>;
    close RD;
    if($one =~ /^RNAplex \d/) {
	unless($opt_q) {
...
sub RNAplex_analysis {
    ...
    # Call it
    (open(PLEX, "$FindBin::RealBin/RNAplex -f 2 -e $option_e -z $int_length < $$qfile |")) || die "ABORT: Failed to open RNAplex main job in sub-routine RNAplex_analysis for query $$query_name\n";
...
sub get_perfect_MFE {
    ...
    (open(PLEX, "$FindBin::RealBin/RNAplex < GSTAr_perfect_tmp_$query_prefix.fasta |")) || die "ABORT: RNAplex call failed in sub-routine get_perfect_MFE\n";
