...
sub blast_run (@) {
	...
	my $blast_out = `$BLAST_BIN -p blastn -d $blastdb -i $queryfile -W 7 -q -1 -S $direction -e 100 -m 8`;
	return $blast_out;
}
...
=head2 print_tab
 print target prediction result as tab delimit format
=cut
sub print_tab {
...
=head1 filter_output_result
 filter the output result by max_mismatch_cutoff, and not_mismatch_position
=cut
...
{
	 my $usage = qq'
USAGE: $0  [options]  miRNA_seq_fasta blast_formatted_db source_db output

	-m	max mismatch cutoff (default: 2)
	-n	not mismatcho position (default: 9,10)
...
sub run_target_score
{
	...
	my ($source_file, $source_file_name, $db_name);
	...
	my $seq_num = 0;
	my $in = Bio::SeqIO->new(-format=>'fasta', -file=>$query_file);	# sRNA is query	
	while(my $inseq = $in->next_seq)
	{
		$seq_num++;
		my $seq_id = $inseq->id;
		...
		print $seq_num."-th input query: ".$seq_id;
	...
	#foreach my $description (@description_list) {
		...
		# tempb is the the temp file of blast result, seq_type is 0 mRNA or 1 mRNA, search direction is both
		# count wil return ??
		...
		#################################################
		my $max_mismatch_cutoff = 2;
		my $not_mismatch_position = "9\t10";

		$max_mismatch_cutoff = int($options{'m'}) if (defined $options{'m'} && $options{'m'} > 0);
		$not_mismatch_position = $options{'n'} if defined $options{'n'};
		$not_mismatch_position =~ s/,/\t/;	

		if ($result_tab) { 
			$result_tab = filter_output_result($result_tab, $max_mismatch_cutoff, $not_mismatch_position); 
			open OUTFILE, ">>$out_file" || die("Cannot Open File");
